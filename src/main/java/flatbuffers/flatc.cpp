...
int main(int argc, const char *argv[]) {
    ...
    flatBufferBuilder.Finish(CreateLogin(flatBufferBuilder));


    //flatbuffers::GetRoot<Login>(buf)
    auto login =GetLogin(flatBufferBuilder.GetBufferPointer());
    std::cout << "loginID="<<login->msgID() << std::endl;
    login->person()->username();
    flatBufferBuilder.ReleaseBufferPointer();
