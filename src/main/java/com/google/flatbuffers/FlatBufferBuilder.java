...
public class FlatBufferBuilder {
    ...
    // Add a scalar to a table at `o` into its vtable, with value `x` and default `_default`
    public void addBoolean(int o, boolean x, boolean _default) {
        if (force_defaults || x != _default) {
            addBoolean(x);
            slot(o);
        }
    }

    public void addByte(int o, byte x, int _default) {
        if (force_defaults || x != _default) {
            addByte(x);
            slot(o);
        }
    ...
    public void addShort(int o, short x, int _default) {
        if (force_defaults || x != _default) {
            addShort(x);
            slot(o);
        }
    ...
    public void addInt(int o, int x, int _default) {
        if (force_defaults || x != _default) {
            addInt(x);
            slot(o);
        }
    ...
    public void addLong(int o, long x, long _default) {
        if (force_defaults || x != _default) {
            addLong(x);
            slot(o);
        }
    ...
    public void addFloat(int o, float x, double _default) {
        if (force_defaults || x != _default) {
            addFloat(x);
            slot(o);
        }
    ...
    public void addDouble(int o, double x, double _default) {
        if (force_defaults || x != _default) {
            addDouble(x);
            slot(o);
        }
    ...
    public void addOffset(int o, int x, int _default) {
        if (force_defaults || x != _default) {
            addOffset(x);
            slot(o);
        }
    ...
    // Structs are stored inline, so nothing additional is being added. `_default` is always 0.
    public void addStruct(int voffset, int x, int _default) {
        if (x != _default) {
            Nested(x);
            slot(voffset);
        }
    ...
    /**
     * The FlatBuffer data doesn't start at offset 0 in the {@link java.nio.ByteBuffer}, but
     * now the {@code ByteBuffer}'s position is set to that location upon {@link #finish(int)}.
     *
     * @return The {@link java.nio.ByteBuffer#position() position} the data starts in {@link #dataBuffer()}
